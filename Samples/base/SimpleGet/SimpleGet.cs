/* @(#) MQMBID sn=p930-L220607.DE su=_T1YMxuZZEeywe89ziwFDLA pn=basedotnet/samples/cs/core/base/SimpleGet/SimpleGet.cs                                               */
/*********************************************************************/
/*   <copyright                                                      */
...
/*                                                                   */
/* Usage:dotnet SimpleGet -q queueName -k keyRepository -s cipherSpec*/
...
namespace SimpleGet
{
    ...
    class SimpleGet
    {
        ...
        static void Main(String[] args)
        {
            ...
            Console.WriteLine("End of SimpleGet Application");
        }
        ...
        void DisplayHelp()
        {
            Console.WriteLine("Usage: SimpleGet -q queueName -k keyRepository -s cipherSpec [-h host -p port -l channel -n numberOfMsgs -dn sslPeerName -kr keyResetCount -cr sslCertRevocationCheck]");
            ...
            Console.WriteLine("Ex: SimpleGet -q QA -k *SYSTEM -s TLS_RSA_WITH_AES_128_CBC_SHA");
            Console.WriteLine("    SimpleGet -q B -k *SYSTEM -s TLS_RSA_WITH_AES_128_CBC_SHA -h remotehost -p 1414 -l SYSTEM.DEF.SVRCONN -dn CN=IBMWEBSPHEREMQQM07 -kr 40000 -cr false");
            Console.WriteLine();
        }
            ...
            try
            {
                ...
                using (var queueManager = CreateQMgrConnection())
                {
                    ...
                    for (int i = 1; i <= noOfMsgs; i++)
                    {
                        ...
                        {
                            queue.Get(message);
                            Console.WriteLine("Message " + i + " Received. Data = " + message.ReadString(message.MessageLength));
                        }
